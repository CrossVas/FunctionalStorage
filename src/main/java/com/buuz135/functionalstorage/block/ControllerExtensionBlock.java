...
public class ControllerExtensionBlock extends RotatableBlock<ControllerExtensionTile> {
    ...
    public IFactory<ControllerExtensionTile> getTileEntityFactory() {
        return () -> new ControllerExtensionTile(this);
    }
    ...
    public ActionResultType use(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand hand, BlockRayTraceResult ray) {
        return TileUtil.getTileEntity(worldIn, pos, ControllerExtensionTile.class).map(drawerTile -> drawerTile.onSlotActivated(player, hand, ray.getDirection(), ray.getLocation().x, ray.getLocation().y, ray.getLocation().z)).orElse(ActionResultType.PASS);
    }
    ...
    public void registerRecipe(Consumer<IFinishedRecipe> consumer) {
        TitaniumShapedRecipeBuilder.shapedRecipe(FunctionalBlocks.CONTROLLER_EXTENSION)
